...
namespace Computer.Win32Class
{
    public class Win32Process : CIMProcess
    {
        ...
        public void Create(string CommandLine)
        {
            Process m_Process = null;
            m_Process = new Process();
            m_Process.StartInfo.FileName = CommandLine;
            m_Process.Start();
        }
        public uint Create(string CommandLine, string CurrentDirectory, out uint ProcessId)
        {
            ManagementScope scope = new ManagementScope(@"\root\cimv2", wmi.Options); 
            scope.Connect();
            ...
            processStartupInstance["CreateFlags"] = 512;//4 | 16; // Create suspended, new window.
            processStartupInstance["ShowWindow"] = 5;
            ...
            inArgs["CommandLine"] = CommandLine;
            inArgs["CurrentDirectory"] = CurrentDirectory;
            inArgs["ProcessStartupInformation"] = processStartupInstance;

            ManagementBaseObject outbo = processClass.InvokeMethod("Create", inArgs, null);
